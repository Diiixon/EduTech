...
public class UsuarioController {
    ...
    public ResponseEntity<Map<String, Object>> login(
            @Parameter(description = "Correo del usuario", example = "usuario@duocuc.cl")
            @RequestParam String correo,
            @Parameter(description = "Clave del usuario", example = "123456")
    ...
    public ResponseEntity<Usuario> registrarUsuario(
            ...
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = "Datos para el registro del usuario (estudiante o empleado)",
                    required = true,
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Usuario actualizdo correctamente",
            content = @Content(schema = @Schema(implementation = Usuario.class))),
            @ApiResponse(responseCode = "404", description = "Usuario no encontrado"),
            @ApiResponse(responseCode = "400", description = "Error en la solicitud")})
    @PutMapping("/{rut}")
    public ResponseEntity<Usuario> actualizarUsuario(
            @Parameter(description = "RUT del usuario que desea actualizar", example = "12345678")
            @PathVariable int rut,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
